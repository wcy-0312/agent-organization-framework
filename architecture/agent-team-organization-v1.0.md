...
### Orchestrator 啟動規則（team-evolution 執行後）

在 team-evolution 執行後進入下一個執行階段前，Orchestrator 必須依序讀取：

1. `.agent-org/current/handoff-package.md`
2. `.agent-org/team-roster.md`
3. `.agent-org/current/team-context-patch.md`（若存在）

`team-context-patch.md` 補充並修正 `handoff-package.md` 中已過時的 team 相關假設。
`team-roster.md` 永遠是角色定義的唯一來源（source of truth）。
`team-context-patch.md` 在下一次 checkpoint closure 時由 `checkpoint-review` 負責歸檔並從 `current/` 移除。
